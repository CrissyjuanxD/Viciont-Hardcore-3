...
public class LegginsNetheriteEssence {
    private final JavaPlugin plugin;

    public LegginsNetheriteEssence(JavaPlugin plugin) {
        this.plugin = plugin;
    }

    public ItemStack createLegginsNetheriteEssence() {
        ItemStack essence = new ItemStack(Material.ECHO_SHARD);
        ItemMeta meta = essence.getItemMeta();

        if (meta != null) {
            // Set display name
            meta.setDisplayName(ChatColor.of("#cc3300") + "" + ChatColor.BOLD + "Leggins Netherite Essence");
